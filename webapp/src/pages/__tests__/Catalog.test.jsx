...
/**
 * Comprehensive Tests for Catalog.jsx
 *
 * This file contains tests for the Catalog component logic.
 * Tests both pure logic functions AND component rendering.
 *
 * Test categories:
 * 1. Pure logic functions (extracted from component)
...
// ============================================================================
// MOCK SETUP - All vi.mock calls must be at the top level
// ============================================================================
...
// Mock useStore - using a factory function pattern
vi.mock('../../store/useStore', () => {
  ...
  return { useStore, __mockState: mockState };
});
  ...
  useTranslation: () => ({
    t: (key, params) => {
      const translations = {
        ...
        'catalog.myShop': '(My Shop)',
        'catalog.selectShop': 'Select a shop',
        'catalog.selectShopDesc': 'Choose a shop to view products',
        'catalog.inStock': 'in stock',
        'catalog.inPreorder': 'in preorder',
        'catalog.searchPlaceholder': 'Search products...',
        'catalog.searchEmpty': 'No products found',
        'catalog.searchEmptyDesc': 'Try different keywords',
        'catalog.searchMinChars': 'Enter at least 2 characters',
        'catalog.tabs.stock': 'In Stock',
        'catalog.tabs.preorder': 'Preorder',
        'catalog.empty': 'No products',
        'catalog.emptyDesc': 'This shop has no products yet',
        'catalog.preorderEmpty': 'No preorders',
        'catalog.preorderEmptyDesc': 'No preorder products available',
        'catalog.backToMyShop': 'Back to my shop',
        'catalog.fromShop': 'from {shop}',
...
vi.mock('../../hooks/useApi', () => ({
  useApi: () => ({
    get: vi.fn().mockResolvedValue({ data: { data: [] }, error: null }),
    post: vi.fn(),
    put: vi.fn(),
    delete: vi.fn(),
  }),
...
vi.mock('../../components/Product/ProductGrid', () => ({
  default: ({ products, loading, emptyTitle, emptyDescription, emptyIcon }) =>
    React.createElement('div', { 'data-testid': 'product-grid' }, [
      loading && React.createElement('div', { key: 'loading', 'data-testid': 'product-grid-loading' }, 'Loading products...'),
      !loading &&
        products.length === 0 &&
        React.createElement('div', { key: 'empty', 'data-testid': 'product-grid-empty' }, [
          React.createElement('span', { key: 'icon' }, emptyIcon),
          React.createElement('h3', { key: 'title' }, emptyTitle),
          React.createElement('p', { key: 'desc' }, emptyDescription),
        ]),
      !loading &&
        products.length > 0 &&
        React.createElement(
          'div',
          { key: 'list', 'data-testid': 'product-list' },
          products.map((product) =>
            React.createElement('div', { key: product.id, 'data-testid': `product-${product.id}` }, `${product.name} - $${product.price}`)
          )
        ),
    ]),
}));
...
vi.mock('../../components/Cart/CartButton', () => ({
  default: ({ onClick }) => React.createElement('button', { 'data-testid': 'cart-button', onClick }, 'Cart'),
}));

vi.mock('../../components/Layout/Header', () => ({
  default: ({ title }) => React.createElement('header', { 'data-testid': 'header' }, title),
}));
...
const mockProducts = [
  { id: 1, name: 'iPhone 15 Pro', price: 999, availability: 'stock', description: 'Latest Apple smartphone' },
  { id: 2, name: 'Samsung Galaxy S24', price: 899, availability: 'stock', description: 'Android flagship' },
  { id: 3, name: 'MacBook Pro M3', price: 1999, availability: 'preorder', description: 'Apple laptop' },
  { id: 4, name: 'iPad Air', price: 599, availability: 'preorder', description: 'Tablet for creativity' },
  { id: 5, name: 'AirPods Pro', price: 249, availability: 'stock', description: 'Wireless earbuds' },
];

const mockShop = { id: 'shop-1', name: 'Tech Store', logo: 'https://example.com/logo.png' };

// ============================================================================
// PURE LOGIC TESTS
// ============================================================================

describe('Catalog - Product Filtering Logic', () => {
  describe('filterProductsByAvailability', () => {
    it('should separate stock and preorder products', () => {
      const result = filterProductsByAvailability(mockProducts);

      expect(result.stockProducts).toHaveLength(3);
      expect(result.preorderProducts).toHaveLength(2);
      expect(result.stockProducts.map((p) => p.id)).toEqual([1, 2, 5]);
      expect(result.preorderProducts.map((p) => p.id)).toEqual([3, 4]);
    });
...
describe('Catalog - Search Functionality Logic', () => {
  describe('searchProducts', () => {
    ...
    it('should find multiple matching products', () => {
      ...
      expect(result.map((p) => p.name)).toContain('MacBook Pro M3');
    });
    ...
    it('should find partial matches in name', () => {
      // "Pro" matches: iPhone 15 Pro, MacBook Pro M3, AirPods Pro
...
describe('Catalog - Price Parsing Logic', () => {
  describe('parsePrice', () => {
    ...
    it('should handle string with currency symbol', () => {
      expect(parsePrice('$99.99')).toBe(0);
    });
...
describe('Catalog - Shop Ownership Detection Logic', () => {
...
describe('Catalog - Edge Cases', () => {
  describe('Product data integrity', () => {
    it('should handle products with extra fields', () => {
      const products = [{ id: 1, name: 'Product', availability: 'stock', extra: 'field', nested: { a: 1 } }];
...
// ============================================================================
// INTEGRATION SCENARIOS (PURE LOGIC)
// ============================================================================
