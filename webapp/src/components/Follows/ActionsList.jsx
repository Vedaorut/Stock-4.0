...
export default function ActionsList({ mode, markup, markupType, onEditMarkup, onSwitchMode, onDelete }) {
  ...
  const switchToLabel = switchToMode === 'monitor' ? t('follows.monitorMode') : t('follows.resellMode');
      ...
      <div>
        ...
        <div className="bg-[#1E1E1E] rounded-3xl overflow-hidden border border-white/5">
            ...
            <>
              ...
              >
                ...
                <div className="flex-1 min-w-0 px-4 text-left">
                  <div className="text-white font-semibold text-[15px]">{t('follows.editMarkup')}</div>
                  <div className="text-white/40 text-xs mt-0.5 font-medium">{t('follows.currentMarkup')} <span className="text-white/60">+{markupType === 'fixed' ? `$${markup}` : `${markup}%`}</span></div>
                </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0 px-4 text-left">
              <div className="text-white font-semibold text-[15px]">{t('follows.switchMode')}</div>
              <div className="text-white/40 text-xs mt-0.5 font-medium">
                {t('follows.switchTo')} <span className="text-blue-400">{switchToLabel.toLowerCase()}</span>
              </div>
            </div>
             <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/20">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div>
        ...
        <div className="bg-[#1E1E1E] rounded-3xl overflow-hidden border border-red-500/10">
          ...
          >
            ...
            <div className="flex-1 min-w-0 px-4 text-left">
              <div className="text-red-500 font-semibold text-[15px]">{t('follows.deleteSubscription')}</div>
              <div className="text-red-500/40 text-xs mt-0.5 font-medium">{t('follows.irreversible')}</div>
            </div>
          </InteractiveListItem>
        </div>
      </div>
