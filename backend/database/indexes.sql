...
CREATE INDEX IF NOT EXISTS idx_orders_payment_status ON orders(payment_hash, status) WHERE payment_hash IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_shop ON subscriptions(user_id, shop_id);
...
CREATE INDEX IF NOT EXISTS idx_payments_hash_status ON payments(tx_hash, status);

-- Index for pending payment verification
CREATE INDEX IF NOT EXISTS idx_payments_pending ON payments(status, updated_at) WHERE status = 'pending';
