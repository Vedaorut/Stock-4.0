...
import { logPaymentsConfig } from './config/payments.js';
...
import { requestTracerMiddleware, errorRequestIdMiddleware } from './middleware/requestTracer.js';
...
/**
 * Request Tracing middleware (API-2: X-Request-ID tracing + metrics)
 * Enhanced with latency tracking and metrics collection
 */
...
/**
 * Load release info (generated by deploy script)
 */
let releaseInfo = { commit_sha: 'dev', build_time: 'local' };
try {
  const fs = await import('fs');
  const releaseFile = new URL('./release.json', import.meta.url);
  if (fs.existsSync(releaseFile)) {
    releaseInfo = JSON.parse(fs.readFileSync(releaseFile, 'utf-8'));
  }
} catch {
  // Dev mode - no release file
}

/**
 * Health check endpoint (API-3: Enhanced with database, memory, and release info)
 */
app.get('/health', async (req, res) => {
  const health = {
    ...
    // Release markers
    commit_sha: releaseInfo.commit_short || releaseInfo.commit_sha?.substring(0, 7) || 'dev',
    build_time: releaseInfo.build_time || 'local',
    memory: {
...
/**
 * Internal status endpoints for monitoring
 */
app.get('/api/internal/status/features', (req, res) => {
  // Import feature flags
  import('./config/payments.js').then(({ PAYMENTS_ENABLED, SUBSCRIPTIONS_ENABLED, ENFORCE_QUOTE_EXPIRY }) => {
    res.json({
      success: true,
      paymentsEnabled: PAYMENTS_ENABLED,
      subscriptionsEnabled: SUBSCRIPTIONS_ENABLED,
      enforceQuoteExpiry: ENFORCE_QUOTE_EXPIRY,
      timestamp: new Date().toISOString(),
    });
  }).catch(() => {
    res.json({
      ...
      enforceQuoteExpiry: true,
      timestamp: new Date().toISOString(),
    });
  });
});

app.get('/api/internal/status/workers', async (req, res) => {
  // Basic worker status - check if worker heartbeat recent
  try {
    const { pool } = await import('./config/database.js');
    // Check if there are any pending payments being processed
    ...
    res.json({
      ...
      worker: {
        ...
        needsReview: parseInt(result.rows[0].needs_review) || 0,
 ...
 * Ensures requestId is in error responses and records error metrics
 */
